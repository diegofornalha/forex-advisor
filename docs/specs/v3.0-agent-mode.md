...
## Evolução: E2B MCP Server (Ideia)

### Contexto

Atualmente o Forex Advisor usa a API REST do E2B (`sandbox.py`). O E2B oferece um **MCP Server** que pode ser integrado diretamente ao Claude Code, oferecendo capacidades mais avançadas.

### Capacidades do E2B MCP Server

| Feature | API REST (atual) | MCP Server (v3.0) |
|---------|------------------|-------------------|
| Execução Python/JS | ✅ | ✅ |
| Instalação dinâmica de pacotes | ❌ | ✅ |
| Acesso à internet no sandbox | ❌ | ✅ |
| Manipulação de arquivos isolada | Limitado | ✅ |
| Comandos terminal | ❌ | ✅ |
| MCP Gateway (200+ servers) | ❌ | ✅ |

### Instalação

```bash
# TypeScript (recomendado)
claude mcp add-json "e2b-server" '{"command":"npx","args":["-y","@e2b/mcp-server"]}'

# Python
claude mcp add-json "e2b-mcp-server" '{"command":"uvx","args":["e2b-mcp-server"]}'
```

### Quando Implementar

- **Após v2.0**: Quando o Agent Mode precisar de capacidades mais avançadas de execução
- **Use case principal**: Tarefas complexas que precisam instalar bibliotecas específicas ou acessar APIs externas durante a execução

### Links

- GitHub: https://github.com/e2b-dev/mcp-server
- Docs: https://e2b.dev/docs/mcp
- MCP Servers: https://mcpservers.org/servers/e2b-dev/mcp-server
